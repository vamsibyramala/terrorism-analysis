      ...
      dcc.Tab(label = "Chart Tool", id="chart tool", value="Chart", children=[
          dcc.Tabs(id = "subtabs2", value = "WorldChart",children = [
              ...
              dcc.Tab(label="India Chart tool", id="IndiaC", value="IndiaChart", children = [
                  html.Br(),
              dcc.Dropdown(id="i_chart", options = chart_dropdown_values, placeholder="Select option", value = "region_txt"),
                  html.Br(),
                  html.Hr(),
                  dcc.Input(id="search-i", placeholder="Search Filter"),
                  html.Hr(),
                  html.Br(),
                  dcc.RangeSlider(
                    id='slider-3',
...
@app.callback(dash.dependencies.Output('graph-object', 'children'),
    ...
    dash.dependencies.Input('i_chart', 'value'),
    dash.dependencies.Input('search-i', 'value'),
...
def update_app_ui(Tabs, month_value, date_value,region_value,country_value,state_value,city_value,attack_value,year_value, chart_year_selector, chart_dp_value, search,
                   chart_i, search_i, year_2,subtabs2):
    fig = None
    ...
    if Tabs == "Map":
        ...
        if subtabs2 == "WorldChart":
        ...
        elif subtabs2 == "IndiaChart":                                      # id india is selcted 
            ...
            if chart_i is not None:
                if search_i is not None: 
                    chart_df = chart_df.groupby("iyear")[chart_i].value_counts().reset_index(name = "count")
                    chart_df  = chart_df[chart_df[chart_i].str.contains(search_i, case = False)]
                else:
                    chart_df = n_df.groupby("iyear")[chart_i].value_counts().reset_index(name="count")
            else:
                raise PreventUpdate
            chartFigure = px.area(chart_df, x= "iyear", y ="count", color = chart_i)
            fig = chartFigure
